...
class LeggedRobot(BaseTask):
    ...
    def _post_physics_step_callback(self):
        ...
        if self.cfg.commands.heading_command and self.cfg.lession.stop==False:
            ...
            if self.cfg.lession.stop:
                self.commands[:, :2] *=(torch.abs(self.commands[:, 4]) > 0.5)
    ...
    def _resample_commands(self, env_ids):#zhiling
        ...
        if self.cfg.lession.stop:
            self.commands[env_ids, :2] *=(torch.abs(self.commands[env_ids, 4]) > 0.5).unsqueeze(1) 
        else:
    ...
    def _reward_foot_clearance1(self):#n coord
        ...
        foot_leteral_vel = torch.sqrt(torch.sum(torch.square(footvel_in_body_frame[:, :, :2]), dim=2)).view(self.num_envs, -1)
        reward=torch.sum(height_error * foot_leteral_vel, dim=1)*(torch.norm(self.commands[:, :2], dim=1) > self.cfg.rewards.command_dead)
    ...
    def _reward_stand_still(self):
        ...
        return torch.sum(torch.abs(self.dof_pos - self.default_dof_pos_st), dim=1) * (torch.norm(self.commands[:, :2], dim=1) <  self.cfg.rewards.command_dead)
    ...
    def _reward_stand_still_force(self):
        ...
        return rew * (torch.norm(self.commands[:, :2], dim=1) <  self.cfg.rewards.command_dead)
    ...
    def _reward_stand_2leg(self):
        ...
        reward_out1= 1.* (double_contact) * (torch.norm(self.commands[:, :2], dim=1) <  self.cfg.rewards.command_dead)
        reward_out2= -0.5* (no_contact) * (torch.norm(self.commands[:, :2], dim=1) <  self.cfg.rewards.command_dead)
        reward_out3= -1.* (single_contact) * (torch.norm(self.commands[:, :2], dim=1) <  self.cfg.rewards.command_dead)
    ...
    def _reward_no_jump(self):
        ...
        single_contact = torch.sum(1.*contacts, dim=1)==1
        no_contact = torch.sum(1.*contacts, dim=1)==0
        reward_out1= 1.* (single_contact) * (torch.norm(self.commands[:, :2], dim=1) >  self.cfg.rewards.command_dead)
        reward_out2= -2.* (no_contact) * (torch.norm(self.commands[:, :2], dim=1) >  self.cfg.rewards.command_dead)
        reward_out3= -0.5* (double_contact) * (torch.norm(self.commands[:, :2], dim=1) >  self.cfg.rewards.command_dead)
        return reward_out1+reward_out2+reward_out3
